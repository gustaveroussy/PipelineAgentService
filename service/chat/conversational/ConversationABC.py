...
class DialogueMedicalABC(StateGraph, ABC):
    """
    对有关复杂医学问题的对话处理器
        1. 防止模型幻觉 RAG
...
class DialogueProcessorABC(StateGraph, ABC):
    ...
    def __init__(self, 
                 ...
                 prompt_file= "config/prompts/DialogueProcessor.yaml"):
    ...
    def compile(self, checkpointer = None, *, store = None, interrupt_before = None, interrupt_after = None, debug = False, name = None):
        ...
        self.graph = super().compile(checkpointer = checkpointer, store=store, interrupt_before=interrupt_before, interrupt_after=interrupt_after, debug=debug, name=name)
        return self.graph
